...
if [ -z ${PDK_ROOT+z} ]; then
    ...
    [ -z "${IIC_OSIC_TOOLS_QUIET}" ] && echo "[INFO] PDK root auto-set to $PDK_ROOT."
fi

if [ -z ${PDK+z} ]; then
    export PDK="gf180mcuD"
    [ -z "${IIC_OSIC_TOOLS_QUIET}" ] && echo "[INFO] PDK set to $PDK."
fi
...
# --- NEW: Mount the PDK ---
PARAMS="$PARAMS -v ${PDK_ROOT}:/foss/pdk:rw -e PDK_ROOT=/foss/pdk -e PDK=${PDK}"
